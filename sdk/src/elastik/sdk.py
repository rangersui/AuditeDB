...
_MAX_DISK_WORLD_NAME_BYTES = 200
...
class TimelineReadResult:
    """Historical timeline body plus the proof headers returned with it."""
    ...
    @property
    def hmac(self) -> str:
        return self.meta["x-timeline-hmac"]


@dataclass(frozen=True, slots=True)
class HeadStamp:
    """Generation-aware audit chain head returned by /proc/audit/*/head."""
...
class Elastik(MutableMapping[str, bytes]):
    ...
    def read_timeline(self, coordinate: TimelineCoordinate) -> TimelineReadResult:
        """GET a historical body and its timeline proof headers."""
        _require_timeline_coordinate(coordinate, "read_timeline")
        ...
        return TimelineReadResult(resp.body, resp.headers)  # type: ignore[arg-type]
    ...
    def chain_head(self, path: str) -> HeadStamp | None:
        """Return the generation-aware audit chain head for a durable path.

        `None` means the path exists but has no audit head, such as a memory
        world. Missing paths and auth failures raise the normal SDK errors.
        """
        world = _canonical_world_name(path)
        _validate_world_name(world)
        target = _audit_head_request_target(world)
        resp = self.request("HEAD", target)
        if resp.status == 204:
            _validate_no_chain_head_response(resp, target)
            return None
        if resp.status >= 400:
            _raise_for_response(resp, "HEAD", target)
        return _head_stamp_from_response(resp, world, target)
    ...
    def listen(
        ...
        *,
        ...
        Events are control-plane only:
          ...
          data: ledger-cursor: var/log/deletes@...
          data: ledger-seq: 2        # subject-world delete pings only
          data: target-generation: 0123456789abcdef0123456789abcdef
          data: event-type: delete_intent
          data: target: /home/task/a
          data: body-sha256: ...
          data: size: 123
          data: content-type: text/plain
          data: reason: ring-miss    # reset events only
        ...
        the handler needs the exact body named by the event. Subject-world
        delete signals are live-only, omit `id`, and include `ledger-cursor`
        plus derived `ledger-seq` for the delete intent row. Durable subject
        deletes include `target-generation`, the deleted incarnation recorded
        in the signed delete-subject proof. The replayable delete-ledger row
        carries the full subject world, generation, seq, body-sha256, and row
        HMAC proof. Subscribe to var/log/deletes for the replayable delete wake
        stream; those events name the ledger world and name the deleted subject
        in `target`.
        `event-type` is the durable audit-row kind when present (`put`,
        `append`, `format`, `delete_intent`, `delete_commit`, or
        `delete_commit_failed`). `body-sha256`, `size`, and `content-type`
        describe the signed audit row; they are metadata, not embedded body
        bytes.
        """
        ...
        if last_event_id is not None:
            if not isinstance(last_event_id, str):
                raise TypeError("last_event_id must be an opaque SSE id string")
...
def _quote_path(path: str) -> str:
    ...
    if world == "proc" or world.startswith("proc/"):
        raise ValueError(
            ...
            "and /proc/audit/{path}/verify or /head exist"
        )
    _validate_world_name(world)
...
def _canonical_proc_path(path: str) -> str | None:
    if path.startswith("//") and path.lstrip("/").startswith("proc"):
        raise ValueError("/proc paths must start with at most one slash")
    ...
    if stripped.endswith("/verify"):
        ...
        raise ValueError("/proc/audit only exposes /proc/audit/{path}/verify or /head")
    raw_world = stripped[len(prefix) : -len(suffix)]
    if not raw_world:
        raise ValueError(f"/proc/audit {label} requires a world path")
    if raw_world.startswith("/") or raw_world.endswith("/") or "//" in raw_world:
        raise ValueError(f"invalid /proc/audit {label} world path")
    world = _canonical_world_name(raw_world)
    _validate_world_name(world)
    return f"proc/audit/{world}{suffix}"
...
def _validate_world_name(world: str) -> None:
    ...
    if _disk_world_name_len(world) > _MAX_DISK_WORLD_NAME_BYTES:
        raise ValueError("elastik path is too long")
...
def _disk_world_name_len(world: str) -> int:
    encoded = 0
    for byte in world.encode("utf-8"):
        encoded += 3 if byte in b'%./\\:*?"<>| ' or byte < 0x20 or byte >= 0x7F else 1
    return encoded
...
def _audit_head_request_target(world: str) -> str:
    return f"/proc/audit/{world}/head"


def _validate_no_chain_head_response(resp: Response, target: str) -> None:
    if resp.headers.get("x-audit-head") != "n/a":
        raise ServerError(
            500,
            b"audit head response status 204 requires x-audit-head n/a\n",
            method="HEAD",
            path=target,
    ...
    proof_headers = [
        name
        for name in ("x-audit-generation", "x-audit-seq", "x-audit-hmac")
        if name in resp.headers
    ]
    if proof_headers:
        joined = ", ".join(proof_headers).encode("utf-8")
        raise ServerError(
            500,
            b"audit head response status 204 must not include proof headers: " + joined + b"\n",
            method="HEAD",
            path=target,
            headers=resp.headers,
        )
...
def _head_stamp_from_response(resp: Response, world: str, target: str) -> HeadStamp:
    if resp.status != 200 or resp.headers.get("x-audit-head") != "true":
        raise ServerError(
            500,
            b"audit head response status must be 200 with x-audit-head true\n",
            method="HEAD",
            path=target,
            headers=resp.headers,
        )
    generation = resp.headers.get("x-audit-generation", "")
    hmac = resp.headers.get("x-audit-hmac", "")
    raw_seq = resp.headers.get("x-audit-seq", "")
    if not _is_lower_hex(generation, 32):
        raise ServerError(
            500,
            b"x-audit-generation missing or invalid\n",
            method="HEAD",
            path=target,
            headers=resp.headers,
        )
    if not (hmac.startswith("hmac-") and _is_lower_hex(hmac[5:], 64)):
        raise ServerError(
            500,
            b"x-audit-hmac missing or invalid\n",
            method="HEAD",
            path=target,
            headers=resp.headers,
        )
    try:
        ...
        raise ServerError(
            500,
            b"x-audit-seq missing or invalid\n",
            method="HEAD",
            path=target,
            headers=resp.headers,
        ) from exc
    ...
    return HeadStamp(world=world, generation=generation, seq=seq, hmac=hmac)
...
def _validate_timeline_response(
    ...
    *,
    ...
    hmac = resp.headers.get("x-timeline-hmac")
    if not hmac or not _is_lower_hex(hmac, 64):
        message = b"timeline proof header x-timeline-hmac missing or invalid\n"
        raise ServerError(
            500,
            message,
            method=method,
...
def _iter_sse(resp) -> Iterator[dict[str, str | None]]:
    event = "message"
