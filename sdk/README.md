...
e.chain_head("note")                  # HEAD /proc/audit/home/note/head
...
`chain_head()` returns the current generation-aware audit head for durable
worlds, or `None` for existing worlds without an audit head. It is an anchoring
primitive; it does not return a subscription cursor or timeline body.
...
- `chain_head()`: generation-aware audit-head probe for external anchoring.
