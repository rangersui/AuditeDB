...
def check_proc_paths_are_sdk_allowed() -> None:
    ...
    assert _quote_path("/proc/audit/home/a b/head") == "/proc/audit/home/a%20b/head"
    assert _quote_path("/proc/audit/home/a b/verify") == "/proc/audit/home/a%20b/verify"
    try:
        _quote_path("//proc/audit/home/a/head")
    except ValueError:
        pass
    else:
        raise AssertionError("SDK accepted double-leading-slash proc path")


def check_proc_audit_empty_segments_are_rejected() -> None:
    for bad in (
        "/proc/audit//home/a/head",
        "/proc/audit/home/a//head",
        "/proc/audit/home//a/head",
        "/proc/audit//home/a/verify",
        "/proc/audit/home/a//verify",
        "/proc/audit/home//a/verify",
    ):
        try:
            ...
            continue
        raise AssertionError(f"SDK accepted invalid proc audit path {bad!r}")
...
def check_overlong_paths_are_rejected_locally() -> None:
    _validate_world_name("home/" + "a" * 185)
    for bad in ("home/" + "a" * 195, "home/" + "a/" * 70):
        try:
            _validate_world_name(bad)
        except ValueError as exc:
            assert "too long" in str(exc), exc
            continue
        raise AssertionError(f"SDK accepted overlong path {bad!r}")
...
def check_timeline_request_uses_query_not_path() -> None:
        ...
        def __init__(
            ...
            *,
            ...
            self.headers = {
                ...
                "X-Timeline-Hmac": "not-hex" if bad_hmac else ("a" * 64),
            }
    ...
    try:
        ...
        result = client.read_timeline(coord)
        assert result.body == b"old"
        assert result.hmac == "a" * 64
        ...
        assert headers["x-timeline-hmac"] == "a" * 64
        assert seen["methods"] == ["GET", "GET", "HEAD"]

        assert "get_timeline" in elastik_module.__all__
        assert "read_timeline" in elastik_module.__all__
        ...
        try:
            ...
            assert elastik_module.read_timeline(coord).hmac == "a" * 64
            assert elastik_module.head_timeline(coord)["x-timeline-seq"] == "7"
            assert seen["methods"] == ["GET", "GET", "HEAD", "GET", "GET", "HEAD"]
        finally:
        ...
        try:
            ...
            assert "x-timeline-hmac missing or invalid" in str(exc), exc
        else:
            raise AssertionError("timeline HMAC proof header mismatch was accepted")
        seen.pop("bad_hmac")
...
def check_chain_head_uses_proc_audit_head() -> None:
    import urllib.request
    ...
    class FakeHTTPResponse:
        def __init__(self) -> None:
            ...
            self.body = b""
            self.headers = {
                "X-Audit-Generation": seen.get("generation", "0" * 32),
                "X-Audit-Seq": seen.get("seq", "7"),
                "X-Audit-Hmac": seen.get("hmac", "hmac-" + "a" * 64),
            }
            if "no_proof" in seen:
                self.headers.pop("X-Audit-Generation", None)
                self.headers.pop("X-Audit-Seq", None)
                self.headers.pop("X-Audit-Hmac", None)
            if "omit_head" not in seen:
                self.headers["X-Audit-Head"] = seen.get("head", "true")
    ...
    try:
        client = Elastik("http://example.test", bearer_token="reader")
        stamp = client.chain_head("home/a b")
        assert isinstance(stamp, HeadStamp)
        assert stamp.world == "home/a b"
        assert stamp.generation == "0" * 32
        assert stamp.seq == 7
        assert stamp.hmac == "hmac-" + "a" * 64
        assert seen["method"] == "HEAD"
        assert seen["auth"] == "Bearer reader"
        assert seen["url"] == "http://example.test/proc/audit/home/a%20b/head"

        seen["status"] = "204"
        seen["head"] = "n/a"
        seen["no_proof"] = "1"
        assert client.chain_head("home/a b") is None
        seen.pop("status")
        seen.pop("head")
        seen.pop("no_proof")
        ...
        seen["head"] = "n/a"
        try:
            client.chain_head("home/a b")
        except ServerError as exc:
            assert "must not include proof headers" in str(exc), exc
        else:
            raise AssertionError("contradictory 204 chain-head response was accepted")
        seen.pop("status")
        seen.pop("head")
        ...
        try:
            ...
            assert "x-audit-head n/a" in str(exc), exc
        else:
            raise AssertionError("malformed 204 chain-head response was accepted")
        seen.pop("status")
        seen.pop("head")
        ...
        try:
            ...
            assert "x-audit-head true" in str(exc), exc
        else:
            raise AssertionError("missing chain-head marker was accepted")
        seen.pop("omit_head")

        for key, value, expected in [
            ("head", "n/a", "x-audit-head true"),
            ("generation", "A" * 32, "x-audit-generation"),
            ("hmac", "a" * 64, "x-audit-hmac"),
            ("hmac", "hmac-" + "G" * 64, "x-audit-hmac"),
            ("seq", "0", "x-audit-seq"),
            ("seq", "9223372036854775808", "x-audit-seq"),
            ("seq", "not-int", "x-audit-seq"),
        ]:
            seen[key] = value
            try:
                client.chain_head("home/a b")
                ...
                raise AssertionError(f"invalid chain-head {key} was accepted")
            seen.pop(key)

        assert "chain_head" in elastik_module.__all__
        elastik_module._set_default(client)
        try:
            assert elastik_module.chain_head("home/a b").seq == 7
        finally:
            elastik_module._default_client = None
    finally:
        urllib.request.urlopen = prior
