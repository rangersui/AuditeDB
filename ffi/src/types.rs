...
use l5::{
    ...
    SubscriptionResetReason, SubscriptionResume, TimelineAddress, TimelineCoordinate,
    TimelineCorruption, TimelineDereference, WorldUsage, WriteKind, WriteResult,
};
...
pub struct FfiTimelineDereference {
    ...
    pub event_body_sha256: Option<String>,
    pub delete_subject_generation: Option<String>,
...
pub struct FfiChangeEvent {
    /// Diagnostic delivery id. Use `cursor`, not this number, for resume.
    pub id: u64,
    /// Durable resume cursor for chain-backed events. Ephemeral events carry
    /// no cursor and must not advance reconnect state.
    ...
    pub delete_ledger_cursor: Option<String>,
    pub delete_ledger_seq: Option<i64>,
    pub delete_subject_generation: Option<String>,
    pub audit_event_type: Option<String>,
    pub audit_event_target: Option<String>,
...
pub struct FfiSubscriptionNext {
    ...
    pub reset_reason: Option<FfiSubscriptionResetReason>,
}

/// Reset reason for a subscription resume boundary.
#[derive(Clone, Copy, Debug, PartialEq, Eq, uniffi::Enum)]
pub enum FfiSubscriptionResetReason {
    Incarnation,
    Truncation,
    RingMiss,
    Memory,
...
pub struct FfiWorldUsage {
    pub world: String,
    pub bytes: u64,
    pub current_body_bytes: u64,
    pub retained_cas_body_bytes: u64,
    pub audit_chain_events: u64,
}
...
pub struct FfiDfSnapshot {
    ...
    pub storage_current_body_bytes: u64,
    pub storage_retained_cas_body_bytes: u64,
    pub storage_audit_chain_events: u64,
    ...
    fn from(value: TimelineDereference) -> Self {
        match value {
            ...
            TimelineDereference::NonBodyEvent(proof) => Self {
                ...
                content_type: Some(proof.content_type().to_owned()),
                size: Some(proof.size()),
                hmac: Some(proof.hmac().to_owned()),
                event_type: Some(proof.event_type().to_owned()),
                event_target: Some(proof.event_target().as_str().to_owned()),
                event_body_sha256: Some(proof.body_sha256().as_str().to_owned()),
                delete_subject_generation: proof
            ...
            TimelineDereference::Expired(proof) => Self {
                kind: FfiTimelineDereferenceKind::Expired,
                representation: None,
                coordinate: Some(FfiTimelineCoordinate::from_timeline_coordinate(
                    proof.requested(),
                )),
...
impl From<WorldUsage> for FfiWorldUsage {
    fn from(value: WorldUsage) -> Self {
        Self {
            ...
            current_body_bytes: value.current_body_bytes as u64,
            retained_cas_body_bytes: value.retained_cas_body_bytes as u64,
            audit_chain_events: value.audit_chain_events as u64,
        }
    }
}
...
impl From<ChangeEvent> for FfiChangeEvent {
    fn from(value: ChangeEvent) -> Self {
        let timeline_address = value.timeline_address().cloned();
        let delete_ledger_event_id = value.delete_ledger_event_id().cloned();
        let cursor = match value.identity() {
            ChangeEventIdentity::Chain(id) => Some(id.to_string()),
            ChangeEventIdentity::Ephemeral => None,
            _ => None,
        };
        Self {
            id: value.id(),
            ...
            delete_ledger_cursor: delete_ledger_event_id.as_ref().map(ToString::to_string),
            delete_ledger_seq: delete_ledger_event_id.as_ref().map(|id| id.seq().get()),
            delete_subject_generation: value
                .delete_subject_generation()
                .map(|generation| generation.as_str().to_owned()),
            audit_event_type: value
                .audit_event_type()
                .map(|kind| kind.as_str().to_owned()),
            audit_event_target: value
                .audit_event_target()
                .map(|target| target.as_str().to_owned()),
            body_sha256: value.body_sha256().map(|hash| hash.as_str().to_owned()),
...
impl From<SubscriptionResetReason> for FfiSubscriptionResetReason {
    fn from(value: SubscriptionResetReason) -> Self {
        match value {
            SubscriptionResetReason::Incarnation => Self::Incarnation,
            SubscriptionResetReason::Truncation => Self::Truncation,
            SubscriptionResetReason::RingMiss => Self::RingMiss,
            SubscriptionResetReason::Memory => Self::Memory,
            _ => Self::Unknown,
        }
    }
}
...
impl TryFrom<FfiSubscriptionResume> for SubscriptionResume {
    ...
    fn try_from(value: FfiSubscriptionResume) -> Result<Self, Self::Error> {
        match value.after_cursor {
            None => Ok(Self::none()),
            Some(raw) => SubscriptionEventId::from_sse_id(&raw)
                .map(Self::after_event_id)
                .map_err(|err| FfiError::InvalidConfig {
...
impl From<DfSnapshot> for FfiDfSnapshot {
    fn from(value: DfSnapshot) -> Self {
        Self {
            ...
            storage_current_body_bytes: value.storage_current_body_bytes as u64,
            storage_retained_cas_body_bytes: value.storage_retained_cas_body_bytes as u64,
            storage_audit_chain_events: value.storage_audit_chain_events as u64,
