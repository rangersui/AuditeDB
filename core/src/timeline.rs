...
use crate::audit::MatchedTimelineRowHmac;
...
/// Positive SQLite `events.id` row coordinate inside one durable world generation.
...
pub struct TimelineBody {
    ...
    hmac: MatchedTimelineRowHmac,
}

/// Verified durable subject named by a delete-ledger row.
///
/// Delete ledger rows store this proof in signed event metadata so a later
/// subscriber can distinguish "this subject path was deleted" from "this exact
/// durable incarnation and body-head was deleted". The address names the
/// deleted subject's latest body event before deletion; `hmac` is that subject
/// event row's HMAC.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct DeleteSubjectProof {
    address: TimelineAddress,
    hmac: String,
}

/// Metadata for an audited historical body whose CAS bytes have expired.
///
/// The row still exists in the verified audit chain, so callers can keep the
/// exact coordinate, declared content type, declared byte size, and row HMAC
/// without falling back to the current body.
pub struct TimelineExpiredBody {
    proof: crate::audit::VerifiedExpiredTimelineBody,
}
...
pub enum TimelineRead {
    ...
    Body(TimelineBody),
    /// The audit row exists, but CAS retention can no longer materialize the
    /// historical body.
    Expired(TimelineExpiredBody),
...
impl TimelineBody {
    ...
    /// Returns the HMAC of the audit row that produced this body.
...
impl TimelineExpiredBody {
    /// Returns the address whose body bytes are outside retained CAS history.
    pub fn address(&self) -> &TimelineAddress {
        self.proof.address()
    }
...
impl TimelineRead {
    ...
    pub(crate) fn expired(proof: crate::audit::VerifiedExpiredTimelineBody) -> Self {
        Self::Expired(TimelineExpiredBody { proof })
    }
}
...
impl DeleteSubjectProof {
    pub(crate) fn new(address: TimelineAddress, hmac: String) -> Self {
        Self { address, hmac }
    }
    ...
    pub(crate) fn from_parts(
        ...
        Self::new(TimelineAddress::new(world, gen, seq, body_sha256), hmac)
    }

    /// Returns the deleted subject's verified body-head address.
    pub fn address(&self) -> &TimelineAddress {
        &self.address
    }

    /// Returns the HMAC of the deleted subject's verified body-head row.
    pub fn hmac(&self) -> &str {
        &self.hmac
    }

    /// Returns the durable-world generation of the deleted subject.
    pub fn generation(&self) -> &WorldGeneration {
...
mod tests {
    ...
    fn timeline_read_body_carries_representation() {
        ...
        let result = TimelineRead::body(
            address.clone(),
            representation,
            MatchedTimelineRowHmac::for_tests("hmac-body-row"),
        );
        ...
        match result {
            TimelineRead::Body(body) => {
                ...
                assert_eq!(body.hmac(), "hmac-body-row");
            }
    ...
    fn timeline_read_body_rejects_hash_mismatch() {
        ...
        let result = TimelineRead::body(
            address.clone(),
            representation,
            MatchedTimelineRowHmac::for_tests("hmac-unused"),
        );
    ...
    fn timeline_read_enumerates_absence_and_mismatch_modes() {
        ...
        for read in reads {
            match read {
                ...
                TimelineRead::Body(_) | TimelineRead::Expired(_) | TimelineRead::Corrupt { .. } => {
