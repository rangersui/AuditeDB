...
pub struct EngineBuilder {
    ...
    retained_body_count: world::RetainedBodyCount,
}
...
impl Default for EngineBuilder {
    fn default() -> Self {
        Self {
            ...
            retained_body_count: world::RetainedBodyCount::default(),
        }
    }
}
...
impl EngineBuilder {
    ...
    pub fn build(self) -> Result<Engine, EngineBuildError> {
        ...
        let startup_file_ops = Arc::new(crate::state::FileOpGate::new());
        let startup_file_op =
            startup_file_ops
                .begin()
        ...
        let delete_ledger_created = durable_usages
            .iter()
            .any(|(world_name, _)| crate::ledger::is_delete_ledger_world(world_name));

        let (events, _) = broadcast::channel(self.listen_replay_max);
        let (shutdown_tx, shutdown_rx) = watch::channel(false);
        let listen_epoch =
            crate::subscription_cursor::SubscriptionEpoch::mint().map_err(|err| {
        ...
        let core = Arc::new(Core {
            ...
            retained_body_count: self.retained_body_count,
            storage_body_bytes: Arc::new(AtomicUsize::new(storage_body_bytes)),
            storage_current_body_bytes: Arc::new(AtomicUsize::new(storage_current_body_bytes)),
            storage_retained_cas_body_bytes: Arc::new(AtomicUsize::new(
...
impl Engine {
    ...
    pub fn shutdown(&self) {
        self.inner.core.shutdown_file_ops_and_drain();
...
mod tests {
    use super::*;
    use crate::engine_types::{Preconditions, Representation, SecretBytes, ValidatedWorldPath};
    use bytes::Bytes;
    ...
    fn zero_numeric_limits_match_env_default_semantics() {
        ...
        assert_eq!(
            engine.inner.core.retained_body_count.get(),
            DEFAULT_RETAINED_BODY_COUNT
        );
    ...
    #[tokio::test]
    async fn shutdown_releases_cached_sqlite_file_handles() {
        let root = temp_root("shutdown-fd-drain");
        let engine = Engine::builder()
            .data_root(root.clone())
            .key(AuditHmacKey::try_from_slice(crate::test_support::TEST_HMAC_KEY).unwrap())
            .build()
            .unwrap();
        let world = ValidatedWorldPath::new("home/shutdown-fd").unwrap();
        ...
        engine
            ...
            .unwrap();
        engine
            .delete(&deleted, Preconditions::none(), AccessTier::Approve)
            .await
            .unwrap();

        engine.shutdown();
        std::fs::remove_dir_all(world::validated_world_dir(&root, &world))
            .expect("shutdown should release cached world database handles");
        let ledger = ValidatedWorldPath::new("var/log/deletes").unwrap();
        std::fs::remove_dir_all(world::validated_world_dir(&root, &ledger))
            .expect("shutdown should release cached delete-ledger database handle");
    ...
    #[test]
    fn build_verifies_retained_cas_bodies_before_returning_engine() {
        let root = temp_root("audit-cas-verify");
        let key = AuditHmacKey::try_from_slice(crate::test_support::TEST_HMAC_KEY).unwrap();
        let world = ValidatedWorldPath::new("home/audit-cas").unwrap();
        let write_result =
            world::write_with_audit_checked(&root, &world, b"body", "text/plain", &[], &key);
        assert!(write_result.is_ok(), "fixture write should succeed");
        let db = world::world_db(&root, "home/audit-cas");
        let conn = rusqlite::Connection::open(db).unwrap();
        conn.execute("UPDATE cas_bodies SET body=x'00'", [])
            .unwrap();
        drop(conn);
        ...
        match result {
            Err(EngineBuildError::Storage { detail, .. }) => {
                assert!(detail.contains("cas_bodies.body does not match body_sha256"));
            }
            other => panic!("expected retained-CAS storage corruption, got {other:?}"),
        }
