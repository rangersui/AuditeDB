...
use super::{verifier::VerifyAccumulator, EventRow, VerifyBreak};
...
impl CasRetentionState {
    ...
    pub(super) fn tail_unchecked() -> Self {
        Self {
...
pub(super) fn load(c: &Connection) -> rusqlite::Result<CasRetentionState> {
    load_inner(c, RetainedCasLoad::HashBodies)
}

/// Load retained-CAS state for durable append authority.
///
/// Durable writes still walk and verify every HMAC-linked audit row before
/// minting `VerifiedAuditTx`. They do not need to re-hash every historical
/// retained body blob on every write; explicit verify/startup owns that full
/// body-integrity scan. The append gate keeps the CAS index shape, floor, row
/// references, and sizes in the proof, and the caller separately verifies the
/// current tail body bytes.
pub(super) fn load_for_append(c: &Connection) -> rusqlite::Result<CasRetentionState> {
    load_inner(c, RetainedCasLoad::IndexShape)
}

enum RetainedCasLoad {
    HashBodies,
    IndexShape,
}

fn load_inner(c: &Connection, mode: RetainedCasLoad) -> rusqlite::Result<CasRetentionState> {
    ...
    let bodies = match mode {
        RetainedCasLoad::HashBodies => load_hashed_bodies(c)?,
        RetainedCasLoad::IndexShape => load_body_index(c)?,
    };
...
fn load_hashed_bodies(
    c: &Connection,
) -> rusqlite::Result<HashMap<BodySha256, RetainedCasBodyInfo>> {
...
fn load_body_index(c: &Connection) -> rusqlite::Result<HashMap<BodySha256, RetainedCasBodyInfo>> {
    let mut bodies = HashMap::new();
    let mut stmt = c.prepare("SELECT body_sha256, length(body) FROM cas_bodies")?;
    let rows = stmt.query_map([], |r| Ok((r.get::<_, String>(0)?, r.get::<_, i64>(1)?)))?;
    for row in rows {
        let (body_sha256, len) = row?;
        if len < 0 {
            return Err(corrupt_error("cas_bodies.body length is negative"));
        }
        let body_sha256 = BodySha256::new(body_sha256)
            .map_err(|_| corrupt_error("invalid cas_bodies.body_sha256"))?;
        bodies.insert(body_sha256, RetainedCasBodyInfo { len });
    }
    Ok(bodies)
}

pub(super) fn verify_tail_body_tx(
    c: &Connection,
    row: &EventRow,
    idx: usize,
) -> rusqlite::Result<Option<VerifyBreak>> {
    let Some(event_type) = AuditEventKind::from_storage(row.event_type.as_str()) else {
        return Ok(None);
    };
    let is_body_event = event_type.class().body_bearing;
    let floor = first_retained_seq(c)?;
    let Some(floor) = floor else {
        if is_body_event {
            return Ok(Some(VerifyBreak {
                break_at: idx,
    ...
    let computed = BodySha256::for_body(&body);
    if !computed.ct_eq_str(body_sha256.as_str()) {
        return Err(corrupt_error("cas_bodies.body does not match body_sha256"));
    }
    let len = i64::try_from(body.len())
        .map_err(|_| corrupt_error("cas_bodies.body length does not fit i64"))?;
    if len != row.size {
        return Ok(Some(VerifyBreak {
            ...
            actual: format!("cas-body-size-{len}"),
        }));
    }
    Ok(None)
}

fn first_retained_seq(c: &Connection) -> rusqlite::Result<Option<TimelineSeq>> {
    c.query_row(
        "SELECT first_retained_seq FROM cas_state WHERE id=1",
        [],
        |r| r.get::<_, Option<i64>>(0),
