...
//!    No fd is alive when physical removal runs.
...
//! `Core::read_world_with_etag` signature (no caller churn). Delete
//! acquires all async locks before it enters the fd-lifetime gate, then
//! installs the tombstone synchronously so no `FileOpPermit` is held
//! across `.await`.
...
impl ReadCache {
    ...
    /// Sync version. Production delete calls this after it has acquired
    /// every async lock it needs, so no fd-lifetime permit crosses `.await`.
    ...
    /// Drain and remove every cached read slot.
    ///
    /// Shutdown uses this to release SQLite file handles owned by the engine.
    /// Each slot is drained behind the slot write guard before removal, so the
    /// same fd-lifetime rule as delete eviction still applies.
    pub(crate) fn drain_all_blocking(&self) {
        let slots: Vec<_> = self
            .read_conns
            .iter()
            .map(|entry| (entry.key().clone(), entry.value().clone()))
            .collect();
        for (world, slot) in slots {
            let mut guard = slot
                .inner
                .write()
            ...
            drop(old);
            drop(guard);
            self.remove_evicted_entry(&world, &slot);
        }
    }
...
mod tests {
    ...
    fn proof() -> crate::blocking_sqlite::BlockingSqlite {
    ...
    fn cache_hit_on_second_read() {
        ...
        write_audited(&dir, world, b"hello", "text/plain", &[]);

        let cache = ReadCache::new(DEFAULT_READ_CACHE_MAX_ENTRIES);

        let r1 = cache
            .cached_read_with_hmac(&mut proof(), &dir, &v(world), &test_key())
            .unwrap();
        assert!(r1.is_some());
    ...
    fn cache_open_rejects_legacy_stage_meta_without_generation() {
        ...
        let err =
            match cache.cached_read_with_hmac(&mut proof(), &dir, &v("home/legacy"), &test_key()) {
    ...
    fn cache_hit_revalidates_generation_before_read() {
        ...
        let err = match cache.cached_read_with_hmac(&mut proof(), &dir, &v(world), &test_key()) {
    ...
    #[test]
    fn cached_read_with_hmac_rejects_tampered_current_audit_chain() {
        let dir = scratch_dir("tampered-current-read");
        let world = "home/tampered-current-read";
        write_audited(&dir, world, b"hello", "text/plain", &[]);
        let cache = ReadCache::new(DEFAULT_READ_CACHE_MAX_ENTRIES);
        ...
        let conn = Connection::open(world::world_db(&dir, world)).unwrap();
        conn.execute("UPDATE events SET hmac='bad' WHERE id=2", [])
            .unwrap();
        drop(conn);

        let result = cache.cached_read_with_hmac(&mut proof(), &dir, &v(world), &test_key());

        assert!(matches!(
            result,
            Ok(Some(Err(crate::audit::AuditError::ChainBroken(_))))
        ));
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn cache_hit_updates_last_access_tick() {
        ...
        for w in ["home/a", "home/b"] {
            write_audited(&dir, w, b"hello", "text/plain", &[]);
        }

        let cache = ReadCache::new(2);
        let _ = cache
            .cached_read_with_hmac(&mut proof(), &dir, &v("home/a"), &test_key())
            .unwrap();
        let a_first = cache
            .read_conns
            .get(&v("home/a"))
        ...
        let _ = cache
            .cached_read_with_hmac(&mut proof(), &dir, &v("home/b"), &test_key())
            .unwrap();
        let b_first = cache
            .read_conns
            .get(&v("home/b"))
    ...
    fn missing_world_returns_none_via_phase3() {
        ...
        let cache = ReadCache::new(DEFAULT_READ_CACHE_MAX_ENTRIES);
        let r = cache
            .cached_read_with_hmac(&mut proof(), &dir, &v("home/none"), &test_key())
            .unwrap();
        assert!(r.is_none());
    ...
    fn install_tombstone_short_circuits_read_to_404() {
        ...
        let world = "home/tomb";
        write_audited(&dir, world, b"x", "text/plain", &[]);
    ...
    fn cap_full_miss_evicts_cold_slot_and_caches_new_world() {
        ...
        for w in ["home/a", "home/b", "home/c"] {
            write_audited(&dir, w, b"x", "text/plain", &[]);
        }
        ...
        let r = cache
            .cached_read_with_hmac(&mut proof(), &dir, &v("home/c"), &test_key())
            .unwrap();
        assert!(r.is_some());
        assert!(
    ...
    fn cache_size_one_evicts_on_every_new_world() {
        ...
        for (idx, w) in worlds.iter().enumerate() {
            write_audited(&dir, w, &[b'a' + idx as u8], "text/plain", &[]);
        }

        let cache = ReadCache::new(1);
        for (idx, w) in worlds.iter().enumerate() {
            let read = cache
                .cached_read_with_hmac(&mut proof(), &dir, &v(w), &test_key())
                .unwrap();
            let (stage, _hmac) = read
                .expect("existing world must not look missing at cap=1")
                .expect("existing world audit chain must verify");
        ...
        for (idx, world) in worlds.into_iter().enumerate() {
            ...
            handles.push(thread::spawn(move || {
                for _ in 0..8 {
                    ...
                    let (stage, _hmac) = read
                        .expect("existing world must not look missing at cache size 1")
                        .expect("existing world audit chain must verify");
                    assert_eq!(stage.body, vec![b'a' + idx as u8]);
                }
            }));
    ...
    fn evicted_slot_reopens_existing_world_instead_of_404() {
        ...
        write_audited(&dir, world, b"still here", "text/plain", &[]);
        ...
        let read = cache
            ...
            .unwrap();
        let (stage, _hmac) = read
            .expect("evicted cache slot must reopen existing world")
            .expect("existing world audit chain must verify");
    ...
    fn eviction_refuses_slots_with_active_read_guards() {
        ...
        let world = "home/busy";
        write_audited(&dir, world, b"busy", "text/plain", &[]);
    ...
    fn try_evict_ready_slot_removes_idle_ready_entry() {
        ...
        let world = "home/idle";
        write_audited(&dir, world, b"idle", "text/plain", &[]);
    ...
    fn try_evict_ready_slot_reports_false_when_map_entry_changed() {
        ...
        write_audited(&dir, world, b"stale", "text/plain", &[]);
    ...
    fn lru_eviction_skips_tombstone_and_evicts_ready_slot() {
        ...
        let cache = ReadCache::new(2);
        let _ = cache
            .cached_read_with_hmac(&mut proof(), &dir, &v("home/ready"), &test_key())
            .unwrap();
        ...
        let tombstone_read = cache
            .cached_read_with_hmac(&mut proof(), &dir, &v("home/tomb"), &test_key())
            .unwrap();
    ...
    fn cached_verify_chain_uses_slot_protocol() {
        ...
        let key = test_key();
        let r1 = cache
            .cached_verify_chain(&mut proof(), &dir, &v(world), &key)
            .unwrap();
    ...
    fn cap_transient_slot_concurrent_readers_safe_under_cleanup() {
        ...
        for w in ["home/a", "home/b", "home/c"] {
            write_audited(&dir, w, b"hello world", "text/plain", &[]);
        }
