...
fn test_core_with_read_cache_max(label: &str, read_cache_max_entries: usize) -> (Core, PathBuf) {
...
fn write_permit_preserves_path_based_approve_gate() {
    ...
    assert!(
        crate::world_ops::authorize_write(&world_path("etc/config"), auth::Tier::Approve).is_ok()
    );
    assert!(
        crate::world_ops::authorize_write(&world_path("home/config"), auth::Tier::Write).is_ok()
    );
}
