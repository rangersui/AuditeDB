...
//!
//! `GET` and `HEAD` on these routes are read-gated when a read token is
...
use crate::{
    ...
    engine_introspection::{AuditVerify, ChainSeq, PoolSnapshot, WorldUsage},
...
pub(crate) async fn proc_df(
    ...
    let body = df_body(DfBodySnapshot {
        storage_used: snapshot.storage_used,
        storage_current_body_bytes: snapshot.storage_current_body_bytes,
        storage_retained_cas_body_bytes: snapshot.storage_retained_cas_body_bytes,
        storage_audit_chain_events: snapshot.storage_audit_chain_events,
        storage_quota: snapshot.storage_quota,
        memory_used: snapshot.memory_used,
        memory_quota: snapshot.memory_quota,
...
pub(crate) async fn proc_audit_verify(
    ...
    let (raw_world, action) = if let Some(raw_world) = audit_path.strip_suffix("/verify") {
        (raw_world, AuditProcAction::Verify)
    } else if let Some(raw_world) = audit_path.strip_suffix("/head") {
        (raw_world, AuditProcAction::Head)
    } else if let Some((raw_world, raw_seq)) = audit_path.rsplit_once("/stamp/") {
        let seq = match parse_audit_stamp_seq(raw_seq) {
            Ok(seq) => seq,
            Err(_) => return bad_request("invalid audit stamp sequence"),
        };
        (raw_world, AuditProcAction::Stamp(seq))
    } else {
        return not_found();
    };
    if raw_world.is_empty() {
        return bad_request(action.missing_world_message());
    }
    if raw_world.starts_with('/') || raw_world.ends_with('/') || raw_world.contains("//") {
        return bad_request(action.invalid_world_message());
    }
    let world = match ValidatedWorldPath::new(canonicalize_path(raw_world)) {
        Ok(world) => world,
        Err(_) => return bad_request(action.invalid_world_message()),
    };

    let tier = state.access_tier_from_headers(&headers);
    let engine = state.engine().clone();
    match action {
        AuditProcAction::Verify => {
        ...
        AuditProcAction::Head => {
            let head = match run_introspection(engine, "audit head", move |engine| {
                engine.chain_head(&world, tier)
            })
            .await
            {
                Ok(head) => head,
            ...
            match head {
                Some(stamp) => audit_head(stamp),
                None => audit_head_not_applicable(),
            }
        }
        AuditProcAction::Stamp(seq) => {
            let stamp = match run_introspection(engine, "audit stamp", move |engine| {
                engine.chain_stamp(&world, seq, tier)
            })
            .await
            {
                Ok(stamp) => stamp,
            ...
            match stamp {
                Some(stamp) => audit_stamp(stamp),
                None => audit_stamp_not_applicable(),
            }
        }
    }
}
...
#[derive(Clone, Copy)]
enum AuditProcAction {
    Verify,
    Head,
    Stamp(ChainSeq),
}

impl AuditProcAction {
    fn missing_world_message(self) -> &'static str {
        match self {
            Self::Verify => "audit verify requires a world path",
            Self::Head => "audit head requires a world path",
            Self::Stamp(_) => "audit stamp requires a world path",
        }
    }

    fn invalid_world_message(self) -> &'static str {
        match self {
            Self::Verify => "invalid audit verify world path",
            Self::Head => "invalid audit head world path",
            Self::Stamp(_) => "invalid audit stamp world path",
        }
    }
}

fn parse_audit_stamp_seq(raw: &str) -> Result<ChainSeq, ()> {
    if raw.is_empty()
        || raw.starts_with('+')
        || raw.starts_with('-')
    ...
    let seq = raw.parse::<i64>().map_err(|_| ())?;
    ChainSeq::new(seq).map_err(|_| ())
}
...
fn du_body_from_usage(sizes: &[WorldUsage]) -> String {
    let sizes: Vec<(String, usize, usize, usize, usize)> = sizes
        .iter()
        .map(|usage| {
            (
                usage.world.as_str().to_owned(),
                usage.bytes,
...
mod tests {
    ...
    #[tokio::test]
    async fn proc_audit_head_reports_chain_head_in_headers() {
        let (engine, dir) = test_engine_for_server("proc-audit-head");
        let world = world_path("home/audit-head");
        engine
            .replace(
                &world,
        ...
        let resp = proc_audit_verify(
            State(state),
            Method::HEAD,
            AxPath("home/audit-head/head".to_owned()),
            HeaderMap::new(),
        )
        .await;
        ...
        assert_eq!(
            resp.headers().get("x-audit-generation").unwrap(),
            expected.generation.as_str()
        );
        assert_eq!(
            resp.headers().get("x-audit-seq").unwrap(),
            &expected.seq.to_string()
        );
        assert_eq!(
            resp.headers().get("x-audit-hmac").unwrap(),
            expected.hmac.as_str()
        );
        assert_eq!(resp.headers().get(header::CONTENT_LENGTH).unwrap(), "0");
    ...
    #[tokio::test]
    async fn proc_audit_stamp_reports_found_and_missing_headers() {
        let (engine, dir) = test_engine_for_server("proc-audit-stamp");
        let world = world_path("home/audit-stamp");
        engine
            .replace(
                &world,
                Representation::new(Bytes::from_static(b"hello"), "text/plain", Vec::new()),
            ...
            .unwrap();
        let expected = engine
            .chain_stamp(&world, ChainSeq::new(2).unwrap(), AccessTier::Read)
            .unwrap()
        ...
        let resp = proc_audit_verify(
            State(state.clone()),
            Method::HEAD,
            AxPath("home/audit-stamp/stamp/2".to_owned()),
            HeaderMap::new(),
        )
        .await;
        ...
        match expected {
            crate::engine_introspection::ChainStampRead::Found(stamp) => {
                assert_eq!(
                    resp.headers().get("x-audit-generation").unwrap(),
            ...
            crate::engine_introspection::ChainStampRead::Missing(_) => {
                panic!("seq 2 should exist")
            }
            _ => panic!("unexpected audit stamp result"),
        }
        assert_eq!(resp.headers().get(header::CONTENT_LENGTH).unwrap(), "0");

        let missing = proc_audit_verify(
            State(state),
            Method::HEAD,
            AxPath("home/audit-stamp/stamp/9".to_owned()),
            HeaderMap::new(),
        )
        .await;

        assert_eq!(missing.status(), StatusCode::NO_CONTENT);
        assert_eq!(missing.headers().get("x-audit-stamp").unwrap(), "n/a");
        assert_eq!(missing.headers().get("x-audit-events").unwrap(), "2");
        assert!(missing.headers().get("x-audit-generation").is_some());
        assert_eq!(missing.headers().get(header::CONTENT_LENGTH).unwrap(), "0");

        let _ = std::fs::remove_dir_all(dir);
    }

    #[tokio::test]
    async fn proc_audit_head_reports_memory_world_not_applicable() {
        let (engine, dir) = test_engine_for_server("proc-audit-head-memory");
        let world = world_path("tmp/head-scratch");
        engine
            .replace(
                &world,
        ...
        let resp = proc_audit_verify(
            State(state),
            Method::HEAD,
            AxPath("tmp/head-scratch/head".to_owned()),
            HeaderMap::new(),
        )
        .await;

        assert_eq!(resp.status(), StatusCode::NO_CONTENT);
        assert_eq!(resp.headers().get("x-audit-head").unwrap(), "n/a");
        assert_eq!(resp.headers().get(header::CONTENT_LENGTH).unwrap(), "0");
    ...
    #[tokio::test]
    async fn proc_audit_head_missing_disk_world_does_not_create_db() {
        let (engine, dir) = test_engine_for_server("proc-audit-head-missing-no-create");
        let db = world_db_path_for_server_tests(&dir, "home/missing-head");
        assert!(!db.exists());

        let resp = proc_audit_verify(
            State(server_state_for_engine_for_tests(engine)),
            Method::HEAD,
            AxPath("home/missing-head/head".to_owned()),
            HeaderMap::new(),
        )
        .await;
    ...
    #[tokio::test]
    async fn proc_audit_head_is_read_gated() {
        let (engine, dir) =
            test_engine_for_server_with_read_token("proc-audit-head-auth", b"reader");
        let world = world_path("home/head-auth");
        engine
            .replace(
                &world,
                Representation::new(Bytes::from_static(b"secret"), "text/plain", Vec::new()),
                Preconditions::none(),
                AccessTier::Write,
            )
            .await
        ...
        let resp = proc_audit_verify(
            State(server_state_for_engine_for_tests(engine)),
            Method::HEAD,
            AxPath("home/head-auth/head".to_owned()),
            HeaderMap::new(),
        )
        .await;
    ...
    #[tokio::test]
    async fn proc_audit_head_rejects_empty_segments_before_canonicalization() {
        let (engine, dir) = test_engine_for_server("proc-audit-head-empty-segments");
        let state = server_state_for_engine_for_tests(engine);
    ...
    #[tokio::test]
    async fn proc_audit_head_advertises_options_and_405() {
        let (engine, dir) = test_engine_for_server("proc-audit-head-methods");
        let state = server_state_for_engine_for_tests(engine);

        let options = proc_audit_verify(
            State(state.clone()),
            Method::OPTIONS,
            AxPath("home/anything/head".to_owned()),
            HeaderMap::new(),
        )
        .await;
        assert_eq!(options.status(), StatusCode::NO_CONTENT);
        assert_eq!(
            options.headers().get(header::ALLOW).unwrap(),
            AUDIT_VERIFY_ALLOW
        );

        let delete = proc_audit_verify(
            State(state),
            Method::DELETE,
            AxPath("home/anything/head".to_owned()),
            HeaderMap::new(),
        ...
        assert_eq!(delete.status(), StatusCode::METHOD_NOT_ALLOWED);
        assert_eq!(
            delete.headers().get(header::ALLOW).unwrap(),
            AUDIT_VERIFY_ALLOW
        );
    ...
    #[tokio::test]
    async fn proc_audit_head_rejects_broken_chain_instead_of_minting_head() {
        let (engine, dir) = test_engine_for_server("proc-audit-head-broken");
        let world = world_path("home/audit-head-broken");
        engine
            .replace(
                &world,
                Representation::new(Bytes::from_static(b"hello"), "text/plain", Vec::new()),
            ...
            .await
            .unwrap();
        let db = world_db_path_for_server_tests(&dir, "home/audit-head-broken");
        ...
        let resp = proc_audit_verify(
            State(state),
            Method::HEAD,
            AxPath("home/audit-head-broken/head".to_owned()),
            HeaderMap::new(),
        )
        .await;
    ...
    async fn proc_du_and_df_report_resource_usage() {
        ...
        assert!(du_body.contains("home/hello\t10\t5\t5\t2\n"));
        assert!(du_body.contains("tmp/scratch\t4\t4\t0\t0\n"));
        ...
        assert_eq!(head.status(), StatusCode::OK);
        assert_eq!(head.headers().get(header::CONTENT_LENGTH).unwrap(), "40");
