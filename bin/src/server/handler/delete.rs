...
mod tests {
    ...
    async fn delete_honors_if_match_before_audit_or_remove() {
        ...
        let events: Vec<String> = stmt
            ...
            .collect::<Result<_, _>>()
            .unwrap();
        assert_eq!(events, vec!["format", "delete_intent", "delete_commit"]);
    ...
    async fn delete_rejects_auth_token_and_append_only_ledger() {
        ...
        engine
            ...
            .await
            .unwrap();
        let ledger_seed = world_path("home/delete-policy-ledger-seed");
        engine
            .replace(
                &ledger_seed,
                Representation::new(
                    Bytes::from_static(b"ledger seed"),
                    "text/plain; charset=utf-8",
                    Vec::new(),
                ),
                Preconditions::none(),
            ...
            .unwrap();
        engine
            .delete(&ledger_seed, Preconditions::none(), AccessTier::Approve)
            .await
            .unwrap();
        let delete_ledger = world_path("var/log/deletes");
