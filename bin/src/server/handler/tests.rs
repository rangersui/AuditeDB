...
#[tokio::test]
async fn put_and_post_reject_public_writes_to_delete_ledger() {
    let (engine, dir) = test_engine_for_server("write-delete-ledger-wall");
    let headers = HeaderMap::new();
    ...
    let put = unwrap_response(
        execute_put_with_engine_state(
            headers.clone(),
            Bytes::from_static(b"fake delete history"),
            AccessTier::Approve,
            ledger.clone(),
            &engine,
            &TraceCtx::disabled(),
    ...
    let post = unwrap_response(
        execute_post_with_engine_state(
            headers,
            Bytes::from_static(b"fake append"),
            AccessTier::Approve,
            ledger,
            &engine,
            &TraceCtx::disabled(),
