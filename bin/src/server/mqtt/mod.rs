...
mod tests {
    ...
    #[tokio::test]
    async fn subscribe_fanout_publishes_event_envelope() {
        ...
        assert!(payload.contains("path: /tmp/sensor/temp"));
        assert!(payload.contains("method: PUT"));
        assert!(!payload.contains("22.1"));
        session.abort_subscriptions();
        let _ = std::fs::remove_dir_all(dir);
    }

    #[tokio::test]
    async fn subscribe_fanout_delete_ping_carries_target_generation() {
        let (engine, dir) = test_engine("mqtt-subscribe-delete-generation");
        let subject = ValidatedWorldPath::new("home/sensor/temp").unwrap();
        engine
            .replace(
                &subject,
                Representation::new(Bytes::from_static(b"22.1"), "text/plain", Vec::new()),
        ...
        let (topic, payload) = loop {
            ...
            let payload = String::from_utf8(publish.payload.to_vec()).unwrap();
            if payload.contains("method: DELETE") {
                break (publish.topic, payload);
            }
        };
        assert_eq!(topic, Bytes::from_static(b"sensor/temp"));
        assert!(payload.contains("path: /home/sensor/temp"));
        assert!(payload.contains("ledger-cursor: var/log/deletes@"));
        assert!(payload.contains("ledger-seq: 2"));
        let generation = payload
            .lines()
            .find_map(|line| line.strip_prefix("target-generation: "))
            .expect("delete fanout should carry target-generation");
    ...
    async fn retained_replay_etag_suppresses_buffered_live_duplicate() {
        ...
        assert!(
            timeout(Duration::from_millis(300), rx.recv())
                .await
                .is_err(),
            "buffered format and live event with the replayed ETag must be suppressed"
        );
        ...
        assert!(payload.contains("path: /home/sensor/temp"));
        assert!(payload.contains("method: PUT"));
        assert!(payload.contains("cursor: home/sensor/temp@"));
    ...
    #[tokio::test]
    async fn subscription_loop_fanout_does_not_read_current_body() {
        let (engine, dir) = test_engine_with_read_token("mqtt-fanout-event-envelope");
        let route = mqtt_filter_to_route("sensor/#").unwrap();
        let subscription = engine
            .subscribe(
                &SubscribePattern::new("home/sensor/*"),
        ...
        let payload = loop {
            ...
            let payload = String::from_utf8(publish.payload.to_vec()).unwrap();
            if payload.contains("method: PUT") {
                break payload;
            }
        };
        assert!(payload.contains("path: /home/sensor/temp"));
        assert!(payload.contains("method: PUT"));
        assert!(
            !payload.contains("22.5"),
            "live fanout must publish the event envelope, not the current body"
        );
        assert_eq!(metrics.snapshot().fanout_drops, 0);
        task.abort();
        let _ = std::fs::remove_dir_all(dir);
    }
    ...
    async fn mqtt_tcp_connect_publish_and_subscribe_use_engine() {
        ...
        let payload = String::from_utf8(fanout.payload.to_vec()).unwrap();
        assert!(payload.contains("path: /tmp/sensor/temp"));
        assert!(payload.contains("method: PUT"));
        assert!(!payload.contains("23.0"));
