...
use crate::{
    ...
    engine_types::{
        AccessTier, ChangeEvent, ChangeEventIdentity, ChangeVerb, EngineSubscription,
        Preconditions, Representation, SubscriptionRecvError, SubscriptionResume,
        ValidatedWorldPath,
    },
};
    ...
    loop {
        let change = match subscription.recv().await {
            ...
            Err(SubscriptionRecvError::Reset { reason }) => {
                mqtt_warn(format_args!(
                    "mqtt: subscription reset required; reason={reason:?}"
                ));
                continue;
            }
        ...
        if should_skip_replayed_live(&ctx.replayed, &change) {
            continue;
        }
        let topic = ctx.route.topic_for_world(change.path());
        let payload = mqtt_change_payload(&change);
...
fn mqtt_change_payload(change: &ChangeEvent) -> Bytes {
    ...
    if let ChangeEventIdentity::Chain(id) = change.identity() {
        data.push_str("\ncursor: ");
    ...
    if let Some(address) = change.timeline_address() {
        if address.world() == change.path() {
            ...
            mqtt_warn(format_args!(
                "mqtt: dropping mismatched timeline address from fanout event; path={} timeline_world={}",
                change.path().as_str(),
                address.world().as_str()
            ));
        }
    }
