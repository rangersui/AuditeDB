...
use crate::{
    ...
    engine_types::{AccessTier, ChangeEvent, ChangeVerb, ValidatedWorldPath},
};
...
pub(super) fn should_skip_replayed_live(
    ...
    {
        Entry::Occupied(_) if change.verb() == ChangeVerb::Format => true,
        Entry::Occupied(entry) if entry.get() == change.etag() => true,
