...
Outbound subscription fanout is always granted as QoS 0. It sends an Engine
change envelope, not the current world body and not a durable per-message
queue. The payload is newline-delimited text with `path`, `method`, optional
`etag`, optional durable `cursor`, timeline fields for body-bearing durable
events, `ledger-cursor` plus derived `ledger-seq` for subject-world delete
pings, `target-generation` for durable subject deletes, and `event-type` /
`target` for delete-ledger rows. `target-generation` is the wire projection of
the signed delete-subject proof; the replayable delete-ledger row carries the
full subject world, generation, seq, body-sha256, and row HMAC proof. Retained
replay still uses MQTT retained-message payloads; live fanout is a wake signal.

Slow clients may miss QoS 0 fanout notifications without a client-visible
resync signal. Durable retained data remains readable from `home/`; durable
delete history remains readable by subscribing to `var/log/deletes` through the
HTTP/SSE Engine surface.
