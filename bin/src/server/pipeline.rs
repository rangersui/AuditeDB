...
pub(crate) enum ErrorReason {
    ...
    PayloadTooLarge,
    QuotaExceeded,
    /// 403 -- a validly addressed internal ledger rejects public writes.
    AppendOnly,
    InsufficientStorage,
    StorageRead,
    TimelineStorageRead,
...
mod tests {
    ...
    fn timeline_event_hmac(data_root: &std::path::Path, world: &str, seq: i64) -> String {
        let db = world_db_path_for_server_tests(data_root, world);
        let conn = rusqlite::Connection::open(db).unwrap();
        conn.query_row("SELECT hmac FROM events WHERE id=?1", [seq], |r| r.get(0))
            .unwrap()
    }
    ...
    fn timeline_test_meta_sha256(content_type: &str, headers: &[(String, String)]) -> String {
        ...
        timeline_test_sha256_field(&mut h, b"content-type", content_type);
        for (name, value) in canonical {
            timeline_test_sha256_field(&mut h, b"header-name", &name);
            timeline_test_sha256_field(&mut h, b"header-value", &value);
        }
        hex::encode(h.finalize())
    }

    fn timeline_test_sha256_field(h: &mut Sha256, label: &[u8], value: &str) {
        h.update(label);
        h.update(b"\0");
    ...
    fn timeline_test_event_hmac(input: TimelineTestEventHmacInput<'_>) -> String {
        ...
        timeline_test_hmac_field(&mut mac, b"world", input.world);
        timeline_test_hmac_field(&mut mac, b"timestamp", input.timestamp);
    ...
    fn rewrite_first_body_event_as_non_body_for_tests(
        ...
        let timestamp: String = conn
            .query_row("SELECT timestamp FROM events WHERE id=?1", [seq], |r| {
                r.get(0)
            })
            .unwrap();
        let subject_hmac: String = conn
            .query_row("SELECT hmac FROM events WHERE id=?1", [seq], |r| r.get(0))
            .unwrap();
        let event_type = "delete_intent";
        let body_sha256 = hex::encode(Sha256::digest(b""));
        let content_type = "";
        let headers = [
            ("auditedb-delete-subject-world".to_owned(), world.to_owned()),
            (
                "auditedb-delete-subject-generation".to_owned(),
                generation.clone(),
            ),
            ("auditedb-delete-subject-seq".to_owned(), seq.to_string()),
            (
                "auditedb-delete-subject-body-sha256".to_owned(),
                body_sha256.clone(),
            ),
            (
                "auditedb-delete-subject-hmac".to_owned(),
                format!("hmac-{subject_hmac}"),
            ),
        ];
        let meta_sha256 = timeline_test_meta_sha256(content_type, &headers);
        ...
        let old_body_sha256: String = conn
            .query_row("SELECT body_sha256 FROM events WHERE id=?1", [seq], |r| {
                r.get(0)
            })
            .unwrap();
        conn.execute("DELETE FROM event_headers WHERE event_id=?1", [seq])
            .unwrap();
        for (name, value) in headers {
            conn.execute(
                "INSERT INTO event_headers(event_id, name, value) VALUES(?1, ?2, ?3)",
                rusqlite::params![seq, name, value],
            )
            .unwrap();
        }
        conn.execute(
        ...
        conn.execute(
            "UPDATE events SET event_type=?1, target=?2, body_sha256=?3, size=0, \
             content_type=?4, meta_sha256=?5, hmac=?6, prev_hmac=?7 WHERE id=?8",
            rusqlite::params![
                event_type,
                world,
                body_sha256,
    ...
    async fn write_body_and_capture_timeline_address_with_tier(
        ...
        loop {
            ...
            if event.verb() == crate::engine_types::ChangeVerb::Replace {
                break event
                    .timeline_address()
                    .cloned()
    ...
    #[test]
    fn validate_path_rejects_worlds_too_long_for_disk_component() {
        ...
        match phase {
            Phase::Error {
                ...
                assert_eq!(reason, "world path is too long");
                assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
            }
            _ => panic!("expected Error::PathInvalid for overlong world"),
        }
    }
    ...
    async fn pipeline_timeline_get_returns_historical_body_and_proof_headers() {
        ...
        let expected_hmac = timeline_event_hmac(&dir, "home/timeline/http", address.seq().get());
        ...
        let expected_seq = address.seq().get().to_string();
        assert_eq!(
            resp.headers().get("x-timeline-seq").unwrap(),
            expected_seq.as_str()
        );
        ...
        assert_eq!(
            resp.headers().get("x-timeline-hmac").unwrap(),
            expected_hmac.as_str()
        );
        assert_eq!(resp.headers().get_all("x-timeline-hmac").iter().count(), 1);
    ...
    #[tokio::test]
    async fn pipeline_timeline_expired_returns_410_with_proof_headers() {
        let (engine, dir) = test_engine_for_server("pipeline-timeline-expired");
        let world = "home/timeline/expired";
        let address =
            write_body_and_capture_timeline_address(&engine, world, b"old", Vec::new()).await;
        let query = timeline_query(&address);
        let expected_hmac = timeline_event_hmac(&dir, world, address.seq().get());
        write_text_world_for_tests(&engine, world, "new").await;
        let db = world_db_path_for_server_tests(&dir, world);
        let conn = rusqlite::Connection::open(db).unwrap();
        conn.execute(
            "DELETE FROM cas_bodies WHERE body_sha256=?1",
            [address.body_sha256().as_str()],
        ...
        assert_eq!(
            resp.headers().get("x-timeline-seq").unwrap(),
            address.seq().get().to_string().as_str()
        );
        assert_eq!(
            resp.headers().get("x-timeline-body-sha256").unwrap(),
            address.body_sha256().as_str()
        ...
        assert_eq!(
            resp.headers()
                .get("x-timeline-expired-content-type")
                .unwrap(),
            "text/plain"
        );
        assert_eq!(resp.headers().get("x-timeline-expired-size").unwrap(), "3");
        assert_eq!(
            resp.headers().get("x-timeline-hmac").unwrap(),
            expected_hmac.as_str()
        );
        assert_eq!(response_text(resp).await, "timeline body expired\n");
    ...
    async fn pipeline_timeline_delete_ledger_does_not_overclaim_earlier_coordinates() {
        ...
        .await;
        assert_eq!(first_address.seq().get(), 2);
        ...
        .await;
        assert_eq!(final_address.seq().get(), 3);
        assert_ne!(
    ...
    async fn pipeline_timeline_head_errors_have_no_body() {
        ...
        let (non_body_generation, non_body_seq) =
            rewrite_first_body_event_as_non_body_for_tests(&dir, "home/timeline/non-body");
        let expected_non_body_hmac =
            timeline_event_hmac(&dir, "home/timeline/non-body", non_body_seq);
        let expected_non_body_hash = hex::encode(Sha256::digest(b""));
        ...
        assert_eq!(
            non_body_headers.get("x-timeline-world").unwrap(),
            "home/timeline/non-body"
        );
        assert_eq!(
            non_body_headers.get("x-timeline-generation").unwrap(),
            non_body_generation.as_str()
        );
        assert_eq!(
            non_body_headers.get("x-timeline-seq").unwrap(),
            non_body_seq.to_string().as_str()
        );
        assert_eq!(
            non_body_headers.get("x-timeline-hmac").unwrap(),
            expected_non_body_hmac.as_str()
        );
        assert_eq!(
            non_body_headers.get("x-timeline-event-type").unwrap(),
            "delete_intent"
        );
        assert_eq!(
            non_body_headers.get("x-timeline-event-target").unwrap(),
            "home/timeline/non-body"
        );
        assert_eq!(
            non_body_headers
                .get("x-timeline-event-body-sha256")
                .unwrap(),
            expected_non_body_hash.as_str()
        );
        assert_eq!(non_body_headers.get("x-timeline-event-size").unwrap(), "0");
        assert_eq!(
            non_body_headers
                .get("x-timeline-event-content-type")
                .unwrap(),
            ""
        );
        assert_eq!(
            non_body_headers
                .get("x-timeline-delete-subject-generation")
                .unwrap(),
            non_body_generation.as_str()
        );
        assert_eq!(
        ...
        let corrupt_address = write_body_and_capture_timeline_address(
            &engine,
            "home/timeline/corrupt-body",
            b"gone",
            Vec::new(),
        )
        .await;
        let corrupt_db = world_db_path_for_server_tests(&dir, "home/timeline/corrupt-body");
        let conn = rusqlite::Connection::open(corrupt_db).unwrap();
        conn.execute(
            "DELETE FROM cas_bodies WHERE body_sha256=?1",
            [corrupt_address.body_sha256().as_str()],
        )
        .unwrap();
        drop(conn);
        let corrupt_query = timeline_query(&corrupt_address);

        let get_corrupt = run(
            Method::GET,
            "/home/timeline/corrupt-body".to_string(),
        ...
        assert_eq!(
            response_text(get_corrupt).await,
            "internal error: timeline storage failure\n"
        );
