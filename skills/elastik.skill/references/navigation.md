...
```bash
curl -sS "$BASE/proc/du"   # one line per world: world<TAB>bytes<LF>
curl -sS "$BASE/proc/df"   # storage / memory / quota / world count
```

`/proc/du` is safe to parse as TSV: world names cannot contain control bytes,
so a tab in the line is always the column separator.
