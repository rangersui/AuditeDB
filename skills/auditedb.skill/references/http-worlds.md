...
Because control bytes are rejected at write time, `/proc/du`'s TSV format is
unambiguous: a legitimate world name cannot contain a tab or newline. Each row
is:

```text
world<TAB>total_body_bytes<TAB>current_body_bytes<TAB>retained_cas_body_bytes<TAB>audit_chain_events<LF>
```
...
/proc/du                        read          plain text TSV:
                                              world, total bytes, current
                                              body bytes, retained CAS bytes,
                                              audit event count
/proc/df                        read          plain text metrics: storage,
                                              storage split gauges, memory,
                                              quota, world count
...
/proc/audit/<world>/head        read          current chain head headers
/proc/audit/<world>/stamp/<n>   read          verified chain HMAC at ordinal n
...
`/proc/audit/<world>/head` returns the current generation-aware audit head.
`/proc/audit/<world>/stamp/<n>` returns the verified HMAC at positive chain
ordinal `n`; this ordinal is not an SSE `id` and not a timeline row id. Existing
durable chains without that ordinal return `204` with observed-event metadata;
memory worlds return `204 n/a`.
...
Persisted metadata is part of the audit contract. Durable audit rows store a
`meta_sha256` over the content type and persisted metadata headers. That frame
is length-prefixed (`label\0len\0value\0`) for the content type, each header
name, and each header value; delimiter bytes inside metadata cannot change the
field boundaries.
...
data: method: DELETE
data: ledger-seq: 2
data: ledger-cursor: var/log/deletes@fedcba9876543210fedcba9876543210=2
...
`format`, `put`, `post`, `delete`, or delete-ledger event on each change.

SSE `id` is a durable subscription replay coordinate:
`<world>@<generation>=<timeline-seq>`. The sequence is the current timeline row
coordinate, not a `ChainSeq` audit-stamp ordinal. A new durable world emits an
initial `format` event before the body write. Subject-world delete pings are
live-only, so they omit `id`; use `ledger-cursor` if you need the replayable
delete-ledger row.
Durable subject deletes include `target-generation`, the deleted incarnation
recorded in the signed delete-subject proof. That proof contains the deleted
subject world, generation, seq, body-sha256, and row HMAC; `ledger-cursor`
points at the replayable delete-ledger row that carries the full proof.
The `body-sha256`, `size`, and `content-type` fields describe the signed audit
row. They are metadata, not an embedded body.
